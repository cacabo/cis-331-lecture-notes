...
Let's apply what we learned last class and discuss the assets, security policy,
and threat model.
...
%eip is the instruction pointer. It stores the address of the next instruction
to be executed.

%esp is the stack pointer and points to the bottom of the stack

%ebp is the frame pointer (also called base pointer) and tells us where the
top of the stack is _for the current frame_. This is _not_ the top of the entire
stack, only the top of current function.

%eax is a register used for storing operands, addresses, etc. This is a
